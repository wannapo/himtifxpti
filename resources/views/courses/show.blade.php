            ...
            @auth
                @if($isEnrolled)
                    <div class="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-8">
                        @if($nextLesson)
<<<<<<< HEAD
                            <a href="{{ route('lessons.show', [$course->slug, $nextLesson->id]) }}" class="inline-block text-center bg-red-600 text-white px-8 py-3.5 font-semibold text-lg hover:bg-red-700 transition-colors rounded-lg shadow-sm w-full md:w-auto border border-red-500">Lanjut Belajar</a>
                        @else
...
>>>>>>> 16daf2ab4b3ba50f6b77b31ce427a4794e96c73c
                ...
                @else
                    <form action="{{ route('enrollments.store', $course->id) }}" method="POST">
                        @csrf
<<<<<<< HEAD
                        <button type="submit" class="inline-block text-center bg-red-600 text-white px-8 py-3.5 font-semibold text-lg hover:bg-red-700 transition-colors rounded-lg shadow-sm w-full md:w-auto border border-red-500">Daftar Sertifikasi</button>
                    </form>
                @endif
            @else
...
<<<<<<< HEAD
                                           class="flex flex-col items-center justify-center p-4 border rounded-xl text-center transition-all duration-200 relative group {{ $isLessonComplete ? 'bg-green-50 border-green-200 hover:border-green-300' : 'bg-white border-slate-200 hover:border-red-300 hover:shadow-sm' }}">
                                            @if($isLessonComplete)
                                            ...
                                            @else
                                                <span class="text-3xl font-extrabold text-slate-300 group-hover:text-red-400 mb-1 transition-colors">{{ $lesson->order_index }}</span>
                                            @endif
